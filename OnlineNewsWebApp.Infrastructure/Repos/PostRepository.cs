...
namespace OnlineNewsWebApp.Infrastructure.Repos
{
    public class PostRepository : GenericRepository<Post>, IPostRepository
    {
        ...
        public IList<Post> GetPostsByCategory(string category)
        {
            ...
            return Context.Posts!.Where(p => p.Category!.Name == category).Include(p => p.Category).Include(p => p.PostTagMaps).ThenInclude(ptm => ptm.Tag).ToList();
        }
        public IList<Post> GetPostsByTag(string tag)
        {
            return Context.Tags!.Where(t => t.Name == tag).SelectMany(t => t.PostTagMaps.Select(ptm => ptm.Post)).Include(p => p.Category).Include(p => p.PostTagMaps).ThenInclude(ptm => ptm.Tag).ToList();
        }
